...
当前状态：统一接入架构全部交付落地。
统一宿主（WcpHost）、20 个逻辑槽位（CustomSlotsMod）、语言物理隔离、Jev 全量双轴审计（9 语 100% 通过）与一键安装器（v0.1.2）已全量落地验证。
注册表 / 槽位规则 / 接管范围 / 磁盘健康 / 安装器测试 89/89 项全部通过；9 语部署包运行时与仓库逐字节一致。
...
| `tools/release/` | 资源包发布流水线：`build_installer.py`（安装器打包与自更新索引）、发布清单与 catalog 维护脚本 |
...
| `Install-WCP-Wordbooks.ps1`、`WordbookHub.psm1`、`catalog.json` | 一键安装器核心：GitHub 发现词书、选择安装、峰值磁盘检查、P1-9 磁盘健康核对、SHA-256 差异更新 |
...
- 磁盘健康自愈：更新时自动通过 `Test-WordbookDiskHealth` 核验本地文件完整度，若音频丢失或词表损坏自动摘除记录并全量重下修复。
- 自更新机制：经 `run-installer.ps1` 启动时自动检查更新，若有新版安装器提示确认后平滑切换，失败不阻断本次安装。
...
一键安装器最新版本为 `wcp-installer-v0.1.2`，支持自动通过 GitHub 发现资源、选择性安装、峰值空间预检、差异化更新与本地磁盘健康检查。版本索引 `release-index.json` 挂在 `wcp-mods-*` release 上，启动器检测到新版本时提示用户确认升级。
