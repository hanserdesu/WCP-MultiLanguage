...
16. 2026-09-23 战斗空选项修复（v1.3.2 / v0.1.9）：游戏内存中的词表落后于已保存的当前词书时，宿主可能保持未激活，旧语言战斗队列由原游戏生成空选项。现在战斗出题前仅在存档词书和原生槽位指纹一致时恢复当前词表，再隔离旧队列；离开接管范围时也不再把异语言旧队列写回。注册表和接管范围回归通过，真实游戏逐语种复测仍待完成。
