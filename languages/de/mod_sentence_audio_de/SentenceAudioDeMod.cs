// WCP Sentence Audio DE — BepInEx 5 插件 (德语变体, 基于 mod_sentence_audio 日语版)
// 功能: 在 每日学习(DatabaseManagerS8) 与 词典查询(DatabaseManagerS17) 的
// 例句旁挂 ▶ 按钮, 点击播放 de_sentence_audio/<md5(de)>.mp3 (由
// D:/ATooManyLanguage/French/tools/gen_sentence_audio_fr.py 生成, 文件名规则两端一致)。
//
// 与日语版 (SentenceAudioMod) 的互斥设计:
//   - ExtractDe 只接受「含拉丁字母 且 不含 CJK/假名」的文本 —— 日语例句由
//     SentenceAudioMod 接管, 德语例句由本插件接管, 英语例句两边都没有本地音频,
...
//   French Profile = 德语词库(猫条版), 8116 词): 只在当前词书就是登记过的德语
...
namespace SentenceAudioDe
{
    [BepInPlugin("dev.hanserdesu.sentaudio.de", "WCP Sentence Audio DE", "1.0.0")]
    public class DeSentenceAudioPlugin : BaseUnityPlugin
    {
        internal static ManualLogSource Log;
        private static DeSentenceAudioPlugin Instance;
        private const float ScanInterval = 0.3f;
        private const string AudioDirName = "de_sentence_audio";

        private AudioSource _audio;
        private ConfigEntry<bool> _enabled;
        private float _nextScan;
        private string _audioDir;
        private readonly Dictionary<Button, DeReadBtnState> _readStates =
            new Dictionary<Button, DeReadBtnState>();
        ...
        void Awake()
        {
            ...
            Instance = this;
            PatchSoundTheWord();
            var go = new GameObject("SentenceAudioDePlayer");
            ...
            Log.LogInfo(string.Format(
                "WCP Sentence Audio DE 1.0.0 loaded, audio dir = {0}", _audioDir));
        }
        ...
        // 当前词书是否为已登记德语书。指纹基于排序后的完整词形集合, 与槽位
        // 无关: 用户把德语书挪到别的自定义槽位也能识别; 词数不同/词表被改动
        ...
        private bool ManagedFrenchBookSelected()
        {
            try
            {
                ...
                bool ok = false;
                if (memory != null && memory.Language == BookProfiles.German)
                {
        ...
        // 把游戏自带的"读例句"按钮改造成德语朗读 + FR 标签。
        // 只有该句有本地德语音频时才接管, 否则完整保留游戏原行为。
            ...
            try
            {
                ...
                for (int k = 0; k < all.Length; k++)
                {
                    ...
                    for (int i = 0; i < arr.Length; i++)
                    {
                        ...
                        if (file == null)
                        {
                            // 本句无德语本地音频: 若该按钮此前被接管(残留
                            // FR 标签/TTS 拦截/额外监听), 必须立即原样撤销,
                            // 否则德语书内翻到无音频词条时按钮会滞留 FR 状态。
                            DeReadBtnState stale;
                        ...
                        // 的德语词书按钮上抑制英文 TTS。这样离开本词书时能无损恢复。
        ...
        // 切出受管德语词书时撤销仅由本插件添加的东西。不清空整个 onClick,
        // 否则会删掉其它词书的运行时监听。
        private void RestoreOtherBookUi()
        {
            ...
            try
            {
                if (_readStates.Count > 0)
                {
                    ...
                    foreach (KeyValuePair<Button, DeReadBtnState> pair in _readStates)
                    {
                        Button b = pair.Key;
                        DeReadBtnState st = pair.Value;
        ...
        private void ReleaseReadState(Button b, DeReadBtnState st)
        {
            ...
            foreach (KeyValuePair<TMP_Text, string> label in st.labels)
            {
                ...
                string frText = label.Value.Replace("读例句", "DE");
        ...
        // 由例句文本(或 TMP 富文本)定位本地德语 mp3
        private string LocalAudio(string raw)
        {
            string fr = ExtractDe(raw);
        ...
        // (底部 UK/US 按钮), 和德语例句叠在一起。除保留原监听外, 按类型名动
        ...
        private void PatchSoundTheWord()
        {
            try
            {
                ...
                var prefix = AccessTools.Method(
                    typeof(DeSentenceAudioPlugin), "SoundTheWordPrefix");
                new Harmony("dev.hanserdesu.sentaudio.de")
        ...
        private void Scan(TMP_Text[] arr)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                ...
                if (show)
                {
                    var spb = btn.GetComponent<DeSentencePlayButton>();
        ...
        {
            var go = new GameObject("SentenceAudioDeBtn",
            ...
            img.color = new Color(0.16f, 0.62f, 0.32f, 0.95f);  // 绿色=德语, 区别日语版蓝
            try
            {
            ...
            var spb = go.AddComponent<DeSentencePlayButton>();
            spb.owner = this;
        ...
        internal static string ExtractDe(string raw)
        {
    ...
    public class DeSentencePlayButton : MonoBehaviour
    {
        public string file;
        public DeSentenceAudioPlugin owner;
