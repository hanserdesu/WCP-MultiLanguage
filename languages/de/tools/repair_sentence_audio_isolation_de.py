# -*- coding: utf-8 -*-
"""只增不删地迁移旧共享目录里的德语句音到 de_sentence_audio。"""
import argparse
import hashlib
import json
import shutil
...
def main() -> int:
    ...
    names = sorted({hashlib.md5(de.encode("utf-8")).hexdigest() + ".mp3"
                    for pairs in master.values() for de, _ in pairs})
    source = WCP / "sentence_audio"
    target = WCP / "de_sentence_audio"
    if not args.dry_run:
