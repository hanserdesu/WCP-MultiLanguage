...
def load_overlap(words):
    """英德同形词清单 — 这些词的英文 pron/sentence2 不允许被覆盖。"""
...
def backup(db: Path):
    ...
    if any(BACKUP_DIR.glob(f'{db.stem}.before_german_{tag}_*')):
        return None
    stamp = time.strftime('%Y%m%d_%H%M%S')
    dst = BACKUP_DIR / f'{db.stem}.before_german_{tag}_{stamp}{db.suffix}'
    shutil.copy2(db, dst)
    print('已备份 ->', dst.name)
    return dst
...
def content_matches(con, words, with_sentence):
    ...
    for start in range(0, len(wl), 500):
        ...
        if with_sentence:
            ...
            expected = Counter((w, f'例句：{de}（{zh}）')
                               for w in chunk for de, zh in words[w]['sentences'])
            if actual != expected:
                return False
    return True
...
def patch(db: Path, words: dict, with_sentence: bool, force=False):
    ...
    for i in range(0, len(wl), 500):
        ...
        if with_sentence:
            sents = []
            for w in chunk:
                for de, zh in words[w]['sentences']:
                    sents.append((w, f'例句：{de}（{zh}）'))
...
def main():
    ...
    for w in skipped:
        del words[w]
    if skipped:
        print(f'跳过英德同形词 {len(skipped)} 个 (英文释义/例句保持原版, '
              '运行时由 DeWordListMod 按词书切换)')
