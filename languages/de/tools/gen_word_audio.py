# -*- coding: utf-8 -*-
"""批量生成德语单词发音 MP3 -> 德语独立 de_word_audio 目录。

DeWordListMod 按 `%USERPROFILE%\AppData\LocalLow\WCP\wcp\de_word_audio\<word>.mp3` 查找
本地音频, 找不到才回退内置(英语向)TTS; 德语词必须本地生成。
...
import edge_tts
from audio_paths_de import native_path, word_audio_dir

sys.stdout.reconfigure(encoding='utf-8')
ROOT = Path(__file__).resolve().parent.parent
VOCAB_DIR = word_audio_dir()
MANIFEST = ROOT / 'output' / 'audio_manifest_de.json'
...
def word_list():
    data = json.loads((ROOT / 'output' / 'german_books.json').read_text(
        encoding='utf-8'))
    order = {'a1': 0, 'a2': 1, 'b1': 2, 'b2': 3}
    seen, out = set(), []
    for lv in sorted(order):
        if lv not in data.get('levels', {}):
            continue
...
async def run(words, manifest, max_n):
    ...
    todo = []
    for w in words:
        dest = native_path(VOCAB_DIR / f'{w}.mp3')
...
def main():
    ...
    lock = MANIFEST.parent / 'gen_word_audio.lock'
