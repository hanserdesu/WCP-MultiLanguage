# -*- coding: utf-8 -*-
"""给自定义词书槽位 4 写德语书名昵称 (仅游戏关闭时执行)。

逆向结论: 书单里的书名 = "自定义词书N（" + SelfBookNameN + ")"。
昵称不参与槽位身份 (ChosenBook_Para 才是)。
槽位 1 (日语) / 槽位 2 (法语) / 槽位 3 (俄语) 的昵称原样保留。

  python tools/rename_books_de.py            # 写入德语昵称
  python tools/rename_books_de.py --restore  # 还原槽位4为默认
"""
import json
import os
import shutil
...
NAMES = {
    'SelfBookName4': '德语词库(猫条版)',
}
DEFAULT = {'SelfBookName4': '空槽位'}
...
def main():
    ...
    n3 = (back.get('SelfBookName3') or {}).get('value')
    print('已写入书名昵称:', '; '.join(changed))
    print('备份:', bak.name, '| 回读校验:', 'PASS' if ok else 'FAIL')
    print(f'槽位1昵称保留: {n1!r} | 槽位2昵称保留: {n2!r} | 槽位3昵称保留: {n3!r}')
    print('游戏内显示: 自定义词书四（德语词库(猫条版)）')
