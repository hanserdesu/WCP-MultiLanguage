# -*- coding: utf-8 -*-
"""批量生成德语例句语音 -> 游戏 persistentDataPath/de_sentence_audio/<md5(de)>.mp3

数据源: data/translations/sentences_master.json {word: [[de, zh], ...]}
文件名: md5(de.encode('utf-8')).hexdigest() — 与 BepInEx 插件
        SentenceAudioDeMod 的取音逻辑严格一致 (小写hex)。
特性: 断点续传(按文件存在), 并发限流, 每句重试3次。
用法: python tools/gen_sentence_audio_de.py [--limit N]
"""
import argparse
import asyncio
import hashlib
...
OUT_DIR = Path(os.path.expandvars(
    r'%USERPROFILE%\AppData\LocalLow\WCP\wcp\de_sentence_audio'))
VOICE = 'de-DE-KatjaNeural'
CONCURRENCY = 12


def fname(de: str) -> str:
    return hashlib.md5(de.encode('utf-8')).hexdigest() + '.mp3'


async def worker(sem, de, ok, fail):
    async with sem:
        path = OUT_DIR / fname(de)
        for attempt in range(3):
            try:
                tts = edge_tts.Communicate(de, VOICE)
...
async def main(limit):
    ...
    async def batched():
        nonlocal done
        tasks = [worker(sem, de, ok, fail) for de in todo]
