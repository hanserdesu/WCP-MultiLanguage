// WCP DE Word List — BepInEx 5 插件 (C# 5 语法) v1.1.0
//
// 目的:
//   A) 德语词书 (自定义词书一~四，匹配 catbar-german-complete 指纹) 与其它词书彻底互不干扰。
//   B) 解决快速测试 / 已学词测试题干与选项严重错位（题干与选项出现旧英语词 / 英语题干配德语选项）核心问题。
//   C) 词表绝不回写成空表 / 过短表，设置安全兜底红线。
//   D) 自选测试(自由选择)的选词界面 (MyParameters.S9CurrentArray_Para, S9extraStudy_Para) 严格过滤非德语词。
//   E) 跨词书启动守卫与基线还原 (DeWL_*)：记录接管字段与原始基线，切换词书时安全还原，绝不污染英语或其它词书。
//   F) 查词面板与音标对齐 (DatabaseManagerS8 / S8checkWordMeaning / ButtonTextTransfer / showTheAnswerS8)。
//   G) 单词发音优先走本地 mp3，防回退内置英文 TTS。
//   H) 离线自愈机制：官方更新覆盖 StreamingAssets 下的 .db 后，后台自动检测探测词并从 de_db_payload 重灌还原。
...
namespace DeWordList
{
    [BepInPlugin("dev.hanserdesu.dewordlist", "WCP DE Word List", "1.2.0")]
    public class DeWordListPlugin : BaseUnityPlugin
    {
        internal const string ReviewRangeType = "复习范围词";

        internal static ManualLogSource Log;
        internal static DeWordListPlugin Instance;
        ...
        // 插件自有的存档键 (DeWL_*)
        private const string OwnedListKey = "DeWL_owned_lists";
        private const string OwnedArrayKey = "DeWL_owned_arrays";
        private const string OwnedBoolKey = "DeWL_owned_bools";
        private const string BakPrefix = "DeWL_bak_";
        private const string OwnedSourceSlotKey = "DeWL_owned_source_slot";
        ...
        void Awake()
        {
            ...
            _allowLegacySharedDbWrites = Config.Bind("Legacy", "AllowSharedDatabaseWrites", false,
                "允许旧版德语插件写入共享 wcpFullEng.db/wcpOnlyWord.db。默认关闭以保持语言资源隔离。");
            ...
            _wordAudioFallback = Config.Bind("Legacy", "WordAudioFallback", false,
                ...
                + "<persistentDataPath>/de_word_audio（迁移期行为，只读）。");
            if (HostTakesOver())
            {
                Log.LogWarning("DEWordList: WcpHost 已接管德语, 旧词表插件不再打补丁 (Legacy/YieldToHost=false 可强制旧模式)。");
                return;
            }
            PatchAll();
        }
        ...
        private static bool HostTakesOver()
        {
            ...
            catch (Exception e)
            {
                ...
                Log.LogWarning("de DEWordList: 宿主接管判定失败, 按旧模式继续打补丁: " + e.Message);
                return false;
            }
        }
        private void PatchAll()
        {
            Harmony harmony = new Harmony("dev.hanserdesu.dewordlist");
            MethodInfo pre = AccessTools.Method(typeof(DeWordListPlugin), "Pre");
            MethodInfo post = AccessTools.Method(typeof(DeWordListPlugin), "Post");
            MethodInfo postRef = AccessTools.Method(typeof(DeWordListPlugin), "PostRef");
            MethodInfo postBook = AccessTools.Method(typeof(DeWordListPlugin), "PostBookChange");
            MethodInfo postS8 = AccessTools.Method(typeof(DeWordListPlugin), "PostSetAsLearnedTest");
            MethodInfo preS9 = AccessTools.Method(typeof(DeWordListPlugin), "PreMcGenS9");
            ...
            MethodInfo preSel = AccessTools.Method(typeof(DeWordListPlugin), "PreFixSelection");
            MethodInfo postSel = AccessTools.Method(typeof(DeWordListPlugin), "PostFixSelection");
            int selCount = 0;
            ...
            stemCount += PatchOne(harmony, "MultipleChoiceGenerator", "GenerateOptions",
                AccessTools.Method(typeof(DeWordListPlugin), "PostMcGen"));
            stemCount += PatchOne(harmony, "MultipleChoiceGeneratorS9", "GenerateOptions",
                AccessTools.Method(typeof(DeWordListPlugin), "PostMcGenS9"));
            // 题面取 need[0]，在游戏读取之前将题干队列摆正
            stemCount += PatchPre(harmony, "SetInputFieldValueS8", "ShowTheWord",
                AccessTools.Method(typeof(DeWordListPlugin), "PreShowTheWord"));
            // 快速测试按钮：重新采样只取本书已学词
            stemCount += PatchOne(harmony, "clickChangeImageSource", "StartQuickTest",
                AccessTools.Method(typeof(DeWordListPlugin), "PostStartQuickTest"));

            // 查词面板
            int dictCount = 0;
            dictCount += PatchOne(harmony, "DatabaseManagerS8", "OnSearchButtonClick",
                AccessTools.Method(typeof(DeWordListPlugin), "PostSearchDictS8"));
            dictCount += PatchOne(harmony, "S8checkWordMeaning", "OnSearchButtonClick",
                AccessTools.Method(typeof(DeWordListPlugin), "PostSearchDictCheck"));
            dictCount += PatchOne(harmony, "ButtonTextTransfer", "OnSearchButtonClick",
                AccessTools.Method(typeof(DeWordListPlugin), "PostSearchDictTransfer"));

            MethodInfo postPhonetic = AccessTools.Method(typeof(DeWordListPlugin), "PostAnswerPhonetic");
            int answerCount = 0;
            ...
            int classifyCount = PatchOne(harmony, "SetInputFieldValueS8", "changeKnownFuzzUnknownTimes",
                AccessTools.Method(typeof(DeWordListPlugin), "PostS8Classification"));

            int audioCount = PatchPre(harmony, "VocabularyAudioPlayer", "PlayWordAudio",
                AccessTools.Method(typeof(DeWordListPlugin), "PrePlayWordAudio"));


            Log.LogInfo("DEWordList: Harmony patches OK: scene=" + preCount + " post=" + postCount +
        ...
        private static void HealTestList()
        {
            ...
            if (bookFixed != null)
            {
                ...
                Warn("测试词池混入非德语词条, 已按德语词表重建 " + bookFixed.Count + " 词");
                return;
            }
        ...
        private static void PostStartQuickTest()
        {
            ...
            try
            {
                ...
                SetTestLists(pool);
                Warn("快速测试词池已按德语书重采样 " + pool.Count + " 词");
            }
        ...
        private static List<string> SampleBookLearned(int target)
        {
            ...
            foreach (KeyValuePair<string, WordInfo> kv in learned)
            {
                ...
                if (!bookSet.Contains(kv.Key)) continue; // 必须是当前德语书中的词
        ...
        private static int BookStateRaw()
        {
            ...
            if (memoryProfile == null || memoryProfile.Language != BookProfiles.German) return 0;
        ...
        // wcpFullEng/wcpOnlyWord 文件；德语插件不能在德语书激活时把它
        ...
        private static void LogBookState(int state)
        {
            ...
            Log.LogInfo("DEWordList: 词书状态=" + state + " 内存书名=" + MyParameters.ChosenBook_Para +
        ...
        private static void RegenerateByGame()
        {
            ...
            if (mgr == null) return;
            mgr.FightList();
            Log.LogInfo("DEWordList: 换出受管词书, 唤醒游戏重新生成当前战斗词表");
        }
        ...
        private static bool PrePlayWordAudio(VocabularyAudioPlayer __instance)
        {
            ...
            if (!System.IO.File.Exists(file) && _wordAudioFallback != null && _wordAudioFallback.Value)
                file = System.IO.Path.Combine(Application.persistentDataPath,
                    "de_word_audio", word + ".mp3");
            if (!System.IO.File.Exists(file))
            {
                WarnOnce("word-audio:" + word, "德语独立单词音频缺失: " + word);
                return false;
            }
        ...
        private IEnumerator PlayLocalWordAudio(string file, int request)
        {
                ...
                new Uri(file).AbsoluteUri, AudioType.MPEG))
            {
                ...
                {
                    WarnOnce("word-decode:" + file, "德语单词音频加载失败: " + web.error);
                    yield break;
                }
        ...
        private static int SelfBookIndexOf(string name)
        {
            ...
            if (!name.StartsWith("自定义词书", StringComparison.Ordinal) &&
                !name.StartsWith("德语词库", StringComparison.Ordinal)) return 0;
        ...
        {
            return profile != null && profile.Language != BookProfiles.German;
        }
        ...
        private static Dictionary<string, string> BookDict()
        {
            ...
            try
            {
                ...
                {
                    Log.LogInfo("DEWordList: 加载德语字典 wordDictionary" + idx + " 共 " +
                                _bookDict.Count + " 词");
                }
            }
            catch (Exception e)
        ...
        private static bool BaselineBelongsToOwnedBook(IEnumerable<string> words)
        {
            ...
            try
            {
                ...
                if (profile == null || profile.Language != BookProfiles.German) return false;
        ...
        private static bool BaselineFromOtherManagedBook(IEnumerable<string> words)
        {
            try
            {
                ...
                for (int i = 0; i < BookProfiles.All.Length; i++)
                {
                    ...
                    if (p.Language == BookProfiles.German) continue;
                    candidates.Add(p);
                }
        ...
        private static bool RestoreSharedFields()
        {
            ...
            ForgetOwned();
            Log.LogInfo("DEWordList: 已还原共享队列" +
        ...
        private static void LogChange(string key, int count, string sample)
        {
            ...
            LastSig[key] = sig;
            Log.LogInfo("DEWordList: " + key + " -> " + count + " 词 (示例: " + sample + ") @" +
                (ManagedBookSelected() ? BookProfiles.German : "other"));
        }
        ...
        private const string DbPackDir = "de_db_payload";

        // 宿主资源包的语言代码: packs/<PackLangCode>/manifest.json
        private const string PackLangCode = "de";
        // 探测词必须同时覆盖两种状态:
        //   être — 德语独有词, 只在补丁库里存在 (官方覆盖后消失)
        //   table — 英法同形词, 稳态/英语书 = 英文原版; 德语书 = 德语释义
        private static readonly string[] DbProbes = new string[] { "gehen", "haben", "müssen" };
        ...
        private static void TickDbHeal()
        {
            ...
            if (!System.IO.Directory.Exists(pack))
            {
                ...
                // TickSharedDbSwitch 会永远停在 heal 等待, 本会话内德语
                // 同形词一直显示英文内容。
                _dbHealTried = true;
                _dbSwitchTried = true;
                return;
        ...
        private static void RunDbHeal(string pack, string full, string only)
        {
            try
            {
                ...
                if (!NeedsDbHeal(full))
                {
                    // 德语独有词都在 = 官方库未被覆盖。同形词当前是英是法
                    // 由 TickSharedDbSwitch 按词书处理, 不属于自愈职责。
                    InfoOnce("dbheal-skip", "德语独有词补丁完整, 不再重灌");
                    _dbSwitchTried = true;
                    return;
                }
                string fullPron = System.IO.Path.Combine(pack, "de_pron.tsv");
                string fullSent = System.IO.Path.Combine(pack, "de_sentences.tsv");
                string onlyPron = System.IO.Path.Combine(pack, "de_only_pron.tsv");
                if (!System.IO.File.Exists(fullPron) || !System.IO.File.Exists(fullSent) ||
                    !System.IO.File.Exists(onlyPron))
                {
                    throw new System.IO.FileNotFoundException("de_db_payload 不完整");
                }
                ...
                if (Log != null)
                {
                    Log.LogInfo("DEWordList: 例句库已自动重灌 FullEng.pron+sent=" + n1 +
        ...
        private static bool EnglishProbeOk(SqliteConnection con)
        {
            using (SqliteCommand cmd = con.CreateCommand())
            {
                ...
                // 英文原版 meaning 以 "(n.) 桌子..." 开头; 德语补丁以 "桌子〈" 开头
        ...
        private static bool ManagedProbeOk(SqliteConnection con)
        {
            using (SqliteCommand cmd = con.CreateCommand())
            {
                ...
                // 仅凭“英语探针不是英语”会把半写入/损坏库误判为德语态。
                // 德语态必须同时有德语独有词，并且同形探针不能仍是英文释义。
                cmd.CommandText = "SELECT meaning FROM pron WHERE word = @w";
                cmd.Parameters.Clear();
                cmd.Parameters.Add(new SqliteParameter("@w", EnProbeWord));
        ...
        // 稳态共享库 = 英文原版 + 德语独有词补丁 (同形词保持英文)。
        // 德语书选中时, 1638 个同形词切成德语释义/例句; 离开时切回英文。
            ...
            if (want == 1 && !_dbSwitchTried)
            {
                ...
                if (!_frPatchOk)
                {
                    WarnOnce("switch-nopatch",
                        "德语补丁不完整且 HealExampleDatabase 已关闭, 共享库保持英语态");
                    return;
                }
                _dbSwitchTried = true;
            }
        ...
        private static void SwitchSharedDb(string pack, string full, string only, int want)
        {
            ...
            if (alreadyOk)
            {
                _dbLang = want;
                if (Log != null)
                    Log.LogInfo("DEWordList: 共享库已是" + (want == 1 ? "德语" : "英语") + "态, 无需切换");
                return;
            }
            if (want == 1)
            {
                // 英语 -> 德语: 同形词切成德语。德语 TSV 由 export_de_db_payload.py
                // 从仓库数据确定性生成 (fr_pron/fr_sentences/fr_only_pron 已含同形词)。
                string frPron = System.IO.Path.Combine(pack, "de_pron.tsv");
                string frSent = System.IO.Path.Combine(pack, "de_sentences.tsv");
                string frOnly = System.IO.Path.Combine(pack, "de_only_pron.tsv");
                ...
                _dbLang = 1;
                if (Log != null)
                    Log.LogInfo("DEWordList: 共享库 -> 德语 (FullEng=" + n1 + ", OnlyWord=" + n2 + ")");
            }
            else
            {
                // 德语 -> 英语: 同形词恢复英文原版
                ...
                _dbLang = 2;
                if (Log != null)
                    Log.LogInfo("DEWordList: 共享库 -> 英语 (FullEng=" + n1 + ", OnlyWord=" + n2 + ")");
            }
        }

        // 英文基线没有例句的同形词 (如 max/pin/respecter/sable) 在切德语时
        // 被灌入了德语例句, en_sentences.tsv 不含它们, ApplyDbPack 的 DELETE
        // 覆盖不到 — 切回英语态时需要显式删除。但差集必须先与同形词集合
        // (en_pron.tsv 词集) 求交: 德语独有词 (être/coing/…) 的例句是共享库
        // 稳态 (英文原版 + 德语独有词补丁) 的一部分, 绝不能删。差集是静态
        // 数据, 只算一次。
        ...
        private static void CleanupOrphanSentences(string full, string enSentPath)
        {
            string packFrSent = System.IO.Path.Combine(
                System.IO.Path.GetDirectoryName(enSentPath), "de_sentences.tsv");
            if (_orphanSentWords == null)
            {
                ...
                {
                    // 无法判定同形词集合时宁可少删 (残留少量同形词德语例句),
                    // 也绝不能把 6478 个德语独有词的稳态例句误删。
        ...
        {
            if (Log != null) Log.LogWarning("DEWordList: " + s);
        }
        ...
        private static void InfoOnce(string key, string s)
        {
            ...
            if (Log != null) Log.LogInfo("DEWordList: " + s);
        }
    }
}
