// WCP Yue Word List — BepInEx 5 插件 (C# 5 语法) v1.2.0
//
// 目的:
//   A) 粤语词书 (自定义词书一~四，匹配 catbar-cantonese-complete 指纹) 与其它词书彻底互不干扰。
//   B) 解决快速测试 / 已学词测试题干与选项严重错位（题干与选项出现旧英语词 / 英语题干配粤语选项）核心问题。
//   C) 词表绝不回写成空表 / 过短表，设置安全兜底红线。
//   D) 自选测试(自由选择)的选词界面 (MyParameters.S9CurrentArray_Para, S9extraStudy_Para) 严格过滤非粤语词。
//   E) 跨词书启动守卫与基线还原 (YueWL_*)：记录接管字段与原始基线，切换词书时安全还原，绝不污染英语或其它词书。
//   F) 查词面板与音标对齐 (DatabaseManagerS8 / S8checkWordMeaning / ButtonTextTransfer / showTheAnswerS8)。
//   G) 单词发音优先走本地 mp3，防回退内置英文 TTS。
//   H) 离线自愈机制：官方更新覆盖 StreamingAssets 下的 .db 后，后台自动检测探测词并从 yue_db_payload 重灌还原。
...
namespace YueWordList
{
    [BepInPlugin("dev.hanserdesu.yuewordlist", "WCP Yue Word List", "1.2.0")]
    public class YueWordListPlugin : BaseUnityPlugin
    {
        internal const string ReviewRangeType = "复习范围词";

        internal static ManualLogSource Log;
        internal static YueWordListPlugin Instance;
        ...
        // 插件自有的存档键 (YueWL_*)
        private const string OwnedListKey = "YueWL_owned_lists";
        private const string OwnedArrayKey = "YueWL_owned_arrays";
        private const string OwnedBoolKey = "YueWL_owned_bools";
        private const string BakPrefix = "YueWL_bak_";
        private const string OwnedSourceSlotKey = "YueWL_owned_source_slot";
        ...
        void Awake()
        {
            ...
            _allowLegacySharedDbWrites = Config.Bind("Legacy", "AllowSharedDatabaseWrites", false,
                "允许旧版粤语插件写入共享 wcpFullEng.db/wcpOnlyWord.db。默认关闭以保持语言资源隔离。");
        ...
        private static bool HostTakesOver()
        {
            ...
            catch (Exception e)
            {
                ...
                Log.LogWarning("yue YueWordList: 宿主接管判定失败, 按旧模式继续打补丁: " + e.Message);
                return false;
            }
        }
        private void PatchAll()
        {
            Harmony harmony = new Harmony("dev.hanserdesu.yuewordlist");
            MethodInfo pre = AccessTools.Method(typeof(YueWordListPlugin), "Pre");
            MethodInfo post = AccessTools.Method(typeof(YueWordListPlugin), "Post");
            MethodInfo postRef = AccessTools.Method(typeof(YueWordListPlugin), "PostRef");
            MethodInfo postBook = AccessTools.Method(typeof(YueWordListPlugin), "PostBookChange");
            MethodInfo postS8 = AccessTools.Method(typeof(YueWordListPlugin), "PostSetAsLearnedTest");
            MethodInfo preS9 = AccessTools.Method(typeof(YueWordListPlugin), "PreMcGenS9");
            ...
            MethodInfo preSel = AccessTools.Method(typeof(YueWordListPlugin), "PreFixSelection");
            MethodInfo postSel = AccessTools.Method(typeof(YueWordListPlugin), "PostFixSelection");
            int selCount = 0;
            ...
            stemCount += PatchOne(harmony, "MultipleChoiceGenerator", "GenerateOptions",
                AccessTools.Method(typeof(YueWordListPlugin), "PostMcGen"));
            stemCount += PatchOne(harmony, "MultipleChoiceGeneratorS9", "GenerateOptions",
                AccessTools.Method(typeof(YueWordListPlugin), "PostMcGenS9"));
            // 题面取 need[0]，在游戏读取之前将题干队列摆正
            stemCount += PatchPre(harmony, "SetInputFieldValueS8", "ShowTheWord",
                AccessTools.Method(typeof(YueWordListPlugin), "PreShowTheWord"));
            // 快速测试按钮：重新采样只取本书已学词
            stemCount += PatchOne(harmony, "clickChangeImageSource", "StartQuickTest",
                AccessTools.Method(typeof(YueWordListPlugin), "PostStartQuickTest"));

            // 查词面板
            int dictCount = 0;
            dictCount += PatchOne(harmony, "DatabaseManagerS8", "OnSearchButtonClick",
                AccessTools.Method(typeof(YueWordListPlugin), "PostSearchDictS8"));
            dictCount += PatchOne(harmony, "S8checkWordMeaning", "OnSearchButtonClick",
                AccessTools.Method(typeof(YueWordListPlugin), "PostSearchDictCheck"));
            dictCount += PatchOne(harmony, "ButtonTextTransfer", "OnSearchButtonClick",
                AccessTools.Method(typeof(YueWordListPlugin), "PostSearchDictTransfer"));

            MethodInfo postPhonetic = AccessTools.Method(typeof(YueWordListPlugin), "PostAnswerPhonetic");
            int answerCount = 0;
            ...
            int classifyCount = PatchOne(harmony, "SetInputFieldValueS8", "changeKnownFuzzUnknownTimes",
                AccessTools.Method(typeof(YueWordListPlugin), "PostS8Classification"));

            int audioCount = PatchPre(harmony, "VocabularyAudioPlayer", "PlayWordAudio",
                AccessTools.Method(typeof(YueWordListPlugin), "PrePlayWordAudio"));


            Log.LogInfo("YueWordList: Harmony patches OK: scene=" + preCount + " post=" + postCount +
        ...
        private static void HealTestList()
        {
            ...
            if (bookFixed != null)
            {
                ...
                Warn("测试词池混入非粤语词条, 已按粤语词表重建 " + bookFixed.Count + " 词");
                return;
            }
        ...
        private static void PostStartQuickTest()
        {
            ...
            try
            {
                ...
                SetTestLists(pool);
                Warn("快速测试词池已按粤语书重采样 " + pool.Count + " 词");
            }
        ...
        private static List<string> SampleBookLearned(int target)
        {
            ...
            foreach (KeyValuePair<string, WordInfo> kv in learned)
            {
                ...
                if (!bookSet.Contains(kv.Key)) continue; // 必须是当前粤语书中的词
        ...
        private static int BookStateRaw()
        {
            ...
            if (memoryProfile == null || memoryProfile.Language != BookProfiles.Cantonese) return 0;
        ...
        // 共享数据库只有一个当前语言态。粤语插件也会维护同一组
        // wcpFullEng/wcpOnlyWord 文件；粤语插件不能在粤语书激活时把它
        ...
        private static void LogBookState(int state)
        {
            ...
            Log.LogInfo("YueWordList: 词书状态=" + state + " 内存书名=" + MyParameters.ChosenBook_Para +
        ...
        private static void RegenerateByGame()
        {
            ...
            if (mgr == null) return;
            mgr.FightList();
            Log.LogInfo("YueWordList: 换出受管词书, 唤醒游戏重新生成当前战斗词表");
        }
        ...
        private IEnumerator PlayLocalWordAudio(string file, int request)
        {
                ...
                new Uri(file).AbsoluteUri, AudioType.MPEG))
            {
                ...
                {
                    WarnOnce("word-decode:" + file, "粤语单词音频加载失败: " + web.error);
                    yield break;
                }
        ...
        private static int SelfBookIndexOf(string name)
        {
            ...
            if (!name.StartsWith("自定义词书", StringComparison.Ordinal) &&
                !name.StartsWith("粤语词库", StringComparison.Ordinal)) return 0;
        ...
        {
            return profile != null && profile.Language != BookProfiles.Cantonese;
        }
        ...
        private static Dictionary<string, string> BookDict()
        {
            ...
            try
            {
                ...
                {
                    Log.LogInfo("YueWordList: 加载粤语字典 wordDictionary" + idx + " 共 " +
                                _bookDict.Count + " 词");
                }
            }
            catch (Exception e)
        ...
        private static bool BaselineBelongsToOwnedBook(IEnumerable<string> words)
        {
            ...
            try
            {
                ...
                if (profile == null || profile.Language != BookProfiles.Cantonese) return false;
        ...
        private static bool BaselineFromOtherManagedBook(IEnumerable<string> words)
        {
            try
            {
                ...
                for (int i = 0; i < BookProfiles.All.Length; i++)
                {
                    ...
                    if (p.Language == BookProfiles.Cantonese) continue;
                    candidates.Add(p);
                }
        ...
        private static bool RestoreSharedFields()
        {
            ...
            ForgetOwned();
            Log.LogInfo("YueWordList: 已还原共享队列" +
        ...
        private static void LogChange(string key, int count, string sample)
        {
            ...
            LastSig[key] = sig;
            Log.LogInfo("YueWordList: " + key + " -> " + count + " 词 (示例: " + sample + ") @" +
                (ManagedBookSelected() ? BookProfiles.Cantonese : "other"));
        }
        ...
        // ---------------- 数据库自愈 (yue_db_payload) ----------------

        private const string DbPackDir = "yue_db_payload";

        // 宿主资源包的语言代码: packs/<PackLangCode>/manifest.json
        private const string PackLangCode = "yue";
        // 探测词必须同时覆盖两种状态:
        //   être — 粤语独有词, 只在补丁库里存在 (官方覆盖后消失)
        //   table — 英法同形词, 稳态/英语书 = 英文原版; 粤语书 = 粤语释义
        private static readonly string[] DbProbes = new string[] { "食", "睇", "搞掂" };
        ...
        private static void TickDbHeal()
        {
            ...
            if (!System.IO.Directory.Exists(pack))
            {
                ...
                // TickSharedDbSwitch 会永远停在 heal 等待, 本会话内粤语
                // 同形词一直显示英文内容。
                _dbHealTried = true;
                _dbSwitchTried = true;
                return;
        ...
        private static void RunDbHeal(string pack, string full, string only)
        {
            try
            {
                ...
                if (!NeedsDbHeal(full))
                {
                    // 粤语独有词都在 = 官方库未被覆盖。同形词当前是英是法
                    // 由 TickSharedDbSwitch 按词书处理, 不属于自愈职责。
                    InfoOnce("dbheal-skip", "粤语独有词补丁完整, 不再重灌");
                    _dbSwitchTried = true;
                    return;
                }
                string fullPron = System.IO.Path.Combine(pack, "yue_pron.tsv");
                string fullSent = System.IO.Path.Combine(pack, "yue_sentences.tsv");
                string onlyPron = System.IO.Path.Combine(pack, "yue_only_pron.tsv");
                if (!System.IO.File.Exists(fullPron) || !System.IO.File.Exists(fullSent) ||
                    !System.IO.File.Exists(onlyPron))
                {
                    throw new System.IO.FileNotFoundException("yue_db_payload 不完整");
                }
                ...
                if (Log != null)
                {
                    Log.LogInfo("YueWordList: 例句库已自动重灌 FullEng.pron+sent=" + n1 +
        ...
        private static bool EnglishProbeOk(SqliteConnection con)
        {
            using (SqliteCommand cmd = con.CreateCommand())
            {
                ...
                // 英文原版 meaning 以 "(n.) 桌子..." 开头; 粤语补丁以 "桌子〈" 开头
        ...
        private static bool ManagedProbeOk(SqliteConnection con)
        {
            using (SqliteCommand cmd = con.CreateCommand())
            {
                ...
                // 仅凭“英语探针不是英语”会把半写入/损坏库误判为粤语态。
                // 粤语态必须同时有粤语独有词，并且同形探针不能仍是英文释义。
                cmd.CommandText = "SELECT meaning FROM pron WHERE word = @w";
                cmd.Parameters.Clear();
                cmd.Parameters.Add(new SqliteParameter("@w", EnProbeWord));
        ...
        // 稳态共享库 = 英文原版 + 粤语独有词补丁 (同形词保持英文)。
        // 粤语书选中时, 1638 个同形词切成粤语释义/例句; 离开时切回英文。
            ...
            if (want == 1 && !_dbSwitchTried)
            {
                ...
                if (!_frPatchOk)
                {
                    WarnOnce("switch-nopatch",
                        "粤语补丁不完整且 HealExampleDatabase 已关闭, 共享库保持英语态");
                    return;
                }
                _dbSwitchTried = true;
            }
        ...
        private static void SwitchSharedDb(string pack, string full, string only, int want)
        {
            ...
            if (alreadyOk)
            {
                _dbLang = want;
                if (Log != null)
                    Log.LogInfo("YueWordList: 共享库已是" + (want == 1 ? "粤语" : "英语") + "态, 无需切换");
                return;
            }
            if (want == 1)
            {
                // 英语 -> 粤语: 同形词切成粤语。粤语 TSV 由 export_yue_db_payload.py
                // 从仓库数据确定性生成 (fr_pron/fr_sentences/fr_only_pron 已含同形词)。
                string frPron = System.IO.Path.Combine(pack, "yue_pron.tsv");
                string frSent = System.IO.Path.Combine(pack, "yue_sentences.tsv");
                string frOnly = System.IO.Path.Combine(pack, "yue_only_pron.tsv");
                ...
                _dbLang = 1;
                if (Log != null)
                    Log.LogInfo("YueWordList: 共享库 -> 粤语 (FullEng=" + n1 + ", OnlyWord=" + n2 + ")");
            }
            else
            {
                // 粤语 -> 英语: 同形词恢复英文原版
                ...
                _dbLang = 2;
                if (Log != null)
                    Log.LogInfo("YueWordList: 共享库 -> 英语 (FullEng=" + n1 + ", OnlyWord=" + n2 + ")");
            }
        }

        // 英文基线没有例句的同形词 (如 max/pin/respecter/sable) 在切粤语时
        // 被灌入了粤语例句, en_sentences.tsv 不含它们, ApplyDbPack 的 DELETE
        // 覆盖不到 — 切回英语态时需要显式删除。但差集必须先与同形词集合
        // (en_pron.tsv 词集) 求交: 粤语独有词 (être/coing/…) 的例句是共享库
        // 稳态 (英文原版 + 粤语独有词补丁) 的一部分, 绝不能删。差集是静态
        // 数据, 只算一次。
        ...
        private static void CleanupOrphanSentences(string full, string enSentPath)
        {
            string packFrSent = System.IO.Path.Combine(
                System.IO.Path.GetDirectoryName(enSentPath), "yue_sentences.tsv");
            if (_orphanSentWords == null)
            {
                ...
                {
                    // 无法判定同形词集合时宁可少删 (残留少量同形词粤语例句),
                    // 也绝不能把 6478 个粤语独有词的稳态例句误删。
        ...
        {
            if (Log != null) Log.LogWarning("YueWordList: " + s);
        }
        ...
        private static void InfoOnce(string key, string s)
        {
            ...
            if (Log != null) Log.LogInfo("YueWordList: " + s);
        }
    }
}
