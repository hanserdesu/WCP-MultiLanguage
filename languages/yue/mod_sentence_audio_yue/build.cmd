@echo off
rem Build WCP Sentence Audio YUE plugin (BepInEx 5, C#5 via NETFX csc)
...
"%CSC%" /nologo /noconfig /nostdlib+ /target:library /langversion:5 /optimize+ %REFS% ^
 /out:"%~dp0SentenceAudioYueMod.dll" "%~dp0SentenceAudioYueMod.cs" "%~dp0BookProfiles.cs"
...
copy /y "%~dp0SentenceAudioYueMod.dll" "%GAME%\BepInEx\plugins\SentenceAudioYueMod.dll" >nul
if errorlevel 1 (
