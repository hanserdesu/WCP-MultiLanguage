// WCP Sentence Audio YUE — BepInEx 5 插件 (粤语变体, 基于 mod_sentence_audio 日语版)
// 功能: 在 每日学习(DatabaseManagerS8) 与 词典查询(DatabaseManagerS17) 的
// 例句旁挂 ▶ 按钮, 点击播放 packs/yue/audio/sentence/<md5(yue)>.mp3 (由
// D:/ATooManyLanguage/French/tools/gen_sentence_audio_fr.py 生成, 文件名规则两端一致)。
//
// 与日语版 (SentenceAudioMod) 的互斥设计:
//   - ExtractYue 只接受「含拉丁字母 且 不含 CJK/假名」的文本 —— 日语例句由
//     SentenceAudioMod 接管, 粤语例句由本插件接管, 英语例句两边都没有本地音频,
...
//   French Profile = 粤语词库(猫条版), 8116 词): 只在当前词书就是登记过的粤语
...
namespace SentenceAudioYue
{
    [BepInPlugin("dev.hanserdesu.sentaudio.yue", "WCP Sentence Audio YUE", "1.0.0")]
    public class YueSentenceAudioPlugin : BaseUnityPlugin
    {
        ...
        void Awake()
        {
            ...
            Instance = this;
            PatchSoundTheWord();
            var go = new GameObject("SentenceAudioYuePlayer");
        ...
        private bool ManagedBookSelected()
        {
            try
            {
                ...
                if (memory == null || memory.Language != BookProfiles.Cantonese) return false;
        ...
        private static int OwnWordCount()
        {
            if (_ownWordCount < 0)
            {
                ...
                for (int i = 0; i < BookProfiles.All.Length; i++)
                    if (BookProfiles.All[i].Language == BookProfiles.Cantonese)
        ...
        // 把游戏自带的"读例句"按钮改造成粤语朗读 + FR 标签。
        // 只有该句有本地粤语音频时才接管, 否则完整保留游戏原行为。
            ...
            try
            {
                ...
                for (int k = 0; k < all.Length; k++)
                {
                    ...
                    for (int i = 0; i < arr.Length; i++)
                    {
                        ...
                        if (file == null)
                        {
                            // 本句无粤语本地音频: 若该按钮此前被接管(残留
                            // FR 标签/TTS 拦截/额外监听), 必须立即原样撤销,
                            // 否则粤语书内翻到无音频词条时按钮会滞留 FR 状态。
                            YueReadBtnState stale;
                        ...
                        // 的粤语词书按钮上抑制英文 TTS。这样离开本词书时能无损恢复。
        ...
        // 切出受管粤语词书时撤销仅由本插件添加的东西。不清空整个 onClick,
        // 否则会删掉其它词书的运行时监听。
        private void RestoreOtherBookUi()
        {
            ...
            try
            {
                if (_readStates.Count > 0)
                {
                    ...
                    foreach (KeyValuePair<Button, YueReadBtnState> pair in _readStates)
                    {
                        Button b = pair.Key;
                        YueReadBtnState st = pair.Value;
        ...
        private void ReleaseReadState(Button b, YueReadBtnState st)
        {
            ...
            foreach (KeyValuePair<TMP_Text, string> label in st.labels)
            {
                ...
                string frText = label.Value.Replace("读例句", "YUE");
        ...
        // 由例句文本(或 TMP 富文本)定位本地粤语 mp3
        private string LocalAudio(string raw)
        {
            string fr = ExtractYue(raw);
        ...
        // 标签 "读例句N" -> "YUE", 其余文字(如快捷键号)保留; 原文先存进
        ...
        private void Relabel(Button b, int i)
        {
            ...
            for (int j = 0; j < texts.Length; j++)
            {
                ...
                if (s.IndexOf("读例句", StringComparison.Ordinal) >= 0)
                {
                    ...
                    texts[j].text = s.Replace("读例句", "YUE");
        ...
        // (底部 UK/US 按钮), 和粤语例句叠在一起。除保留原监听外, 按类型名动
        ...
        private void Scan(TMP_Text[] arr)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                ...
                if (show)
                {
                    var spb = btn.GetComponent<YueSentencePlayButton>();
        ...
        {
            var go = new GameObject("SentenceAudioYueBtn",
            ...
            img.color = new Color(0.16f, 0.62f, 0.32f, 0.95f);  // 绿色=粤语, 区别日语版蓝
            try
            {
            ...
            var spb = go.AddComponent<YueSentencePlayButton>();
            spb.owner = this;
        ...
        internal static string ExtractYue(string raw)
        {
    ...
    public class YueSentencePlayButton : MonoBehaviour
    {
        public string file;
        public YueSentenceAudioPlugin owner;
