# -*- coding: utf-8 -*-
"""Portuguese wordbook full audit (verify_all_pt.py).
Mirrors verify_all_ar.py checks. Usage: python tools/verify_all_pt.py
"""
...
OUT = ROOT / 'output'
IMPORT = OUT / 'import'
ES_WORD_AUDIO = Path.home() / 'AppData' / 'LocalLow' / 'WCP' / 'wcp' / 'pt_word_audio'
ES_SENT_AUDIO = Path.home() / 'AppData' / 'LocalLow' / 'WCP' / 'wcp' / 'pt_sentence_audio'
...
def main():
    print('=' * 70)
    print('    Portuguese CEPT-8 / CEFR Wordbook Full Audit')
    print('=' * 70)
    books_p = OUT / 'portuguese_books.json'
    if not check('portuguese_books.json exists', books_p.exists()):
        sys.exit(1)
    ...
    check('level distribution correct',
          {lv: len(levels.get(lv, [])) for lv in ('pt_a1a2', 'pt_b1', 'pt_b2', 'pt_c1')} ==
          {'pt_a1a2': 2000, 'pt_b1': 2500, 'pt_b2': 3000, 'pt_c1': 1100})
    ...
    # import files
    for fn in ['葡萄牙语专四基础.xlsx', '葡萄牙语专四进阶.xlsx', '葡萄牙语专八高阶.xlsx',
               '葡萄牙语专八精通.xlsx', '葡萄牙语考级词库(猫条版).xlsx', 'wcp_portuguese.db']:
        check(f'import file {fn}', (IMPORT / fn).exists())
    # db
    db_p = IMPORT / 'wcp_portuguese.db'
    if db_p.exists():
        conn = sqlite3.connect(db_p)
        c = conn.cursor()
        c.execute('SELECT count(*) FROM pron')
        check('pron count', c.fetchone()[0] == 8600)
        c.execute('SELECT count(*) FROM portuguese_all')
        check('portuguese_all count', c.fetchone()[0] == 8600)
    ...
    if ES_SENT_AUDIO.exists():
        ...
        def norm_pt(raw): return re.sub(r'<[^>]+>', '', raw or '').strip()
        sent_uni = {norm_pt(pair[0]) for v in master2.values() for pair in v
                    if len(pair) == 2 and len(norm_pt(pair[0])) >= 2}
