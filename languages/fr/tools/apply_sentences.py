...
{word: [[fr, zh], ...]} (每词3条)。只纳入通过 gen_pipeline_fr 校验的批次;
...
import gen_pipeline_fr as gp  # noqa: E402
...
def main():
    ...
    for n, y, sl in ps:
        ...
        for meta in sl:
            master[meta['word']] = [[it['fr'], it['zh']]
                                    for it in d[meta['word']][:3]]
    total_sent = sum(len(v) for v in master.values())
    unique = {fr for v in master.values() for fr, _ in v}
