...
def require_game_closed():
    """拒绝在 wcp.exe 运行时改写存档类文件 (见 IMPLEMENTATION.md 4.1.2)。

    tasklist 的「没有匹配任务」提示在不同语言/版本下会写到 stdout 或 stderr,
    因此判定只看「是否有 wcp.exe 那一行」; 拿不到输出则按无法确认处理 (拒绝写入),
    二进制缺失或执行失败也转成同一条可读错误, 而不是抛裸 FileNotFoundError。
    """
