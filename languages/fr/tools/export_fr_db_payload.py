# -*- coding: utf-8 -*-
r"""将游戏词库 (wcpFullEng.db / wcpOnlyWord.db) 里的法文词条导出成持久化离线自愈补丁包。

背景: 官方更新会覆盖 StreamingAssets 下的 .db 文件，导致我们灌进去的法语释义与例句丢失。
MyBook.es3 在 AppData\LocalLow\WCP\wcp 中，更新不会被覆盖，因此词书本身保留，但例句与查词会丢失。

本脚本导出五份 TSV 补丁包并计算哈希 manifest:
  fr_pron.tsv      word / ukPhonic / usPhonic / meaning    (wcpFullEng.db)
  fr_sentences.tsv word / sentences                        (wcpFullEng.db)
  fr_only_pron.tsv word / ukPhonic / usPhonic / meaning    (wcpOnlyWord.db)
  en_pron.tsv      英法同形词英文原版 pron                  (english_baseline)
  en_sentences.tsv 英法同形词英文原版 sentence2             (english_baseline)

英法同形词 (1638 个) 的英文释义/例句不允许被法语覆盖。运行时
FrWordListMod 按当前词书在这批词上双向切换英/法内容, 因此 payload
必须同时携带两套基线。共享库稳态 = 英文原版 + 法语独有词补丁;
法语书选中时插件把同形词切成法语, 离开时切回英文。

注意: fr_pron/fr_sentences/fr_only_pron 仍覆盖全部 8116 词 (含同形
词的法语态), 用于法语书场景 (TickSharedDbSwitch 与 RunDbHeal)。
en_pron/en_sentences 仅含同形词英文原版, 数据源是法语灌库前的
原始库副本 (work/english_baseline), 用于还原英语态。

存放到 LocalLow/fr_db_payload 与 output/fr_db_payload。FrWordListMod 启动时后台检测探测词，
...
def fr_words():
    books_p = ROOT / 'output' / 'french_books.json'
    if books_p.exists():
...
def main():
    ...
    for lv in ('a1', 'a2', 'b1', 'b2'):
        for w in books['levels'][lv]:
            ...
            for fr_s, zh_s in master.get(w['word'], []):
                full_sent.append((w['word'], u'例句：%s（%s）' % (fr_s, zh_s)))
    full_pron.sort(key=lambda r: r[0])
    full_sent.sort(key=lambda r: (r[0], r[1]))
    only_pron.sort(key=lambda r: r[0])
    # 英文基线 (仅英法同形词) — 数据源是法语灌库前的原始库副本
    ...
    for d in targets:
        ...
        write_tsv(d / 'fr_pron.tsv', full_pron)
        write_tsv(d / 'fr_sentences.tsv', full_sent)
        write_tsv(d / 'fr_only_pron.tsv', only_pron)
        if en_pron:
            write_tsv(d / 'en_pron.tsv', en_pron)
        if en_sent:
            write_tsv(d / 'en_sentences.tsv', en_sent)
        print('已生成 ->', d)
    names = ['fr_pron.tsv', 'fr_sentences.tsv', 'fr_only_pron.tsv']
    if en_pron:
