...
function Ensure-ZipExtractor {
    if ('WcpFrenchZipSession' -as [type]) { return }
...
public sealed class WcpFrenchZipSession
{
    public Task Task;
    ...
    public static WcpFrenchZipSession Start(string zipPath, string destination, int workerCount)
    {
        WcpFrenchZipSession session = new WcpFrenchZipSession();
...
function Expand-ZipWithProgress([string]$zipPath, [string]$destination, [string]$displayName) {
    ...
    $session = [WcpFrenchZipSession]::Start($zipPath, $destination, $workers)
...
foreach ($name in @('FrWordListMod.dll', 'BookNameMod.dll', 'SentenceAudioFrMod.dll')) {
...
$repairDir = Join-Path $data 'fr_db_payload'
New-Item -ItemType Directory -Force -Path $repairDir | Out-Null
Get-ChildItem -LiteralPath (Join-Path $payload 'fr_db_payload') -File | ForEach-Object {
    Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $repairDir $_.Name) -Force
}
# 法语单词音频必须落进插件读取的私有目录 (FrWordListMod 拦截 VocabularyAudioPlayer
# 后从 fr_word_audio 播放)。绝不能写英语/日语共用的 LocalLow\WCP\vocabulary ——
# 8,116 词里 1,638 个与英语原库同形, 写进共享目录会让英/日语词书听到法语发音。
New-Item -ItemType Directory -Force -Path (Join-Path $data 'fr_word_audio') | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $data 'fr_sentence_audio') | Out-Null
Copy-TreeNet (Join-Path $audioStage 'vocabulary') (Join-Path $data 'fr_word_audio') 'words'
Copy-TreeNet (Join-Path $audioStage 'sentence_audio') (Join-Path $data 'fr_sentence_audio') 'sentences'
try {
...
$bookPayloadPath = Join-Path $payload 'catbar_french_book.json'
if (-not (Test-Path -LiteralPath $bookPayloadPath)) { Fail '安装包缺少 catbar_french_book.json。' }
...
# 2. 安全写入法语词书与当前选中词书（完整保留全部存档元数据与 __type）
Write-Step '正在写入法语词书并设置当前选中词书。'
if (Test-Path -LiteralPath $myBookPath) {
    $targetSlot = [WcpEs3Helper]::InstallBookAndSave($myBookPath, $savePath, $bookPayloadPath, 2)
    Write-Host "已写入法语词书槽位 $targetSlot，已安全保留全部存档元数据并选中该词书。"
} else {
