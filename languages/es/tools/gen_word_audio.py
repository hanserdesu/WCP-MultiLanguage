# -*- coding: utf-8 -*-
"""Batch generate Spanish word audio MP3 -> es_word_audio/<word>.mp3
edge-tts es-ES-ElviraNeural. Manifest-based resume + async concurrency.
Usage: python tools/gen_word_audio.py [--limit N] [--retry-failed]
"""
import argparse, asyncio, json, os, random, re, sys, time
from pathlib import Path
import edge_tts
from audio_paths_es import word_audio_dir

sys.stdout.reconfigure(encoding='utf-8')
ROOT = Path(__file__).resolve().parent.parent
VOCAB_DIR = word_audio_dir()
MANIFEST = ROOT / 'output' / 'audio_manifest_es.json'
...
async def main():
    parser = argparse.ArgumentParser(description='Generate Spanish word audio')
