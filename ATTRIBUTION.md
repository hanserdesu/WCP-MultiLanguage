...
本仓库包含本项目编写的代码与内容，以及从第三方词典数据集派生的逐项数值。三类材料适用不同许可，范围以本文件为准。
...
适用：从第三方词典数据集提取或机械转换得到的逐项数值，即音标、重音标注、汉字音训读等，例如各语言载荷与 `packs/*/db/` 中的音标列。这些值来自第三方数据集，按上游要求以 CC BY-SA 4.0 提供，署名与相同方式共享两项必须保留。
